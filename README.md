...
本アプリケーションは複雑な環境構築や複数ファイルの連携を必要とせず、**単一のPythonファイル**で動作します。

### 実行手順

1. 本リポジトリのコード（例: `BTC_App.py`）をお手元のPCにコピー、またはダウンロードして保存します。
2. ターミナルを開き、以下の必須ライブラリをインストールします。
   ```bash
...
3. 保存したスクリプトを実行します。
   ...
   py BTC_App.py
   ```
4. GUI画面が立ち上がります。「分析＆稼働開始」ボタンをクリックするとシミュレーションが開始されます。

> **⚠️ Note (免責事項)**
> 本プログラムは技術的なデモンストレーションおよびシミュレーションを目的としたものです。既存のシステム内容では実際の取引所APIのPrivateエンドポイント（実際の発注処理）は叩かない安全な設計となっています。
